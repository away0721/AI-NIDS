...
RESULT_DIR = Path(r"D:\Projects\ai-nids\results\multiclass\cnn_torch_weighted_clipped")
MODEL_PATH = Path(
...
LABEL_ENCODER_PATH = Path(
    r"D:\Projects\ai-nids\models\multiclass\cnn_torch_weighted_clipped_label_encoder.pkl"
)

REPORT_PATH = RESULT_DIR / "cnn_torch_weighted_clipped_multiclass_report.txt"
METRICS_CSV_PATH = RESULT_DIR / "cnn_torch_weighted_clipped_multiclass_metrics.csv"
METRICS_FIG_PATH = RESULT_DIR / "cnn_torch_weighted_clipped_multiclass_metrics.png"
CONFUSION_MATRIX_FIG_PATH = (
    RESULT_DIR / "cnn_torch_weighted_clipped_multiclass_confusion_matrix.png"
)
TRAINING_HISTORY_CSV_PATH = (
    RESULT_DIR / "cnn_torch_weighted_clipped_multiclass_training_history.csv"
)
TRAINING_HISTORY_FIG_PATH = (
    RESULT_DIR / "cnn_torch_weighted_clipped_multiclass_training_history.png"
)
...
def plot_metrics(metrics: dict) -> None:
    ...
    plt.ylim(0, 1)
    plt.title("Clipped Weighted PyTorch CNN Multiclass Metrics")
    plt.ylabel("Score")
...
def plot_confusion_matrix(cm, class_names) -> None:
    ...
    plt.imshow(cm)
    plt.title("Clipped Weighted PyTorch CNN Multiclass Confusion Matrix")
...
def plot_training_history(history_df: pd.DataFrame) -> None:
    ...
    plt.title("Clipped Weighted PyTorch CNN Multiclass Training History")
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.legend()
    plt.tight_layout()
...
def main() -> None:
    ...
    class_weights = np.clip(
        class_weights,
        a_min=None,
        a_max=MAX_CLASS_WEIGHT,
    )

    print(f"裁剪后的类别权重，最大权重限制为：{MAX_CLASS_WEIGHT}")
    ...
    with open(REPORT_PATH, "w", encoding="utf-8") as file:
        file.write("Clipped Weighted PyTorch CNN Multiclass Classification Report\n")
        file.write("=" * 60 + "\n\n")
        file.write(f"Max Class Weight: {MAX_CLASS_WEIGHT}\n")
